...
const teus = require('./client/teus');
...
/**
 * GenerateAddress returns a new Timeus address and its corresponding secret key
...
/**
 * isValidAddress takes an Timeus address and checks if valid.
 * @param addr Timeus address
...
/**
 * mnemonicToSecretKey takes a mnemonic string and returns the corresponding Timeus address and its secret key.
 * @param mn 25 words Timeus mnemonic
 * @returns {{sk: Uint8Array, addr: string}}
...
/**
 * secretKeyToMnemonic takes an Timeus secret key and returns the corresponding mnemonic.
 * @param sk Uint8Array
...
function signTransaction(txn, sk) {
    ...
    let teusTxn = new txnBuilder.Transaction(txn);

    return {"txID": teusTxn.txID().toString(), "blob": teusTxn.signTxn(sk)};
}

/**
 * signBid takes an object with the following fields: bidder key, bid amount, max price, bid ID, auctionKey, auction ID,
 * and a secret key and returns a signed blob to be inserted into a transaction Timeus note field.
 * @param bid Timeus Bid
 * @param sk Timeus secret key
 ...
 * @param addrs a list of Timeus addresses representing possible signers for this multisig. Order is important.
 * @param sk Timeus secret key. The corresponding pk should be in the pre image.
...
function signMultisigTransaction(txn, {version, threshold, addrs}, sk) {
    ...
    let teusTxn = new multisig.MultiSigTransaction(txn);
    const pks = addrs.map(addr => {
        return address.decode(addr).publicKey;
    });
    return {
        "txID": teusTxn.txID().toString(),
        "blob": teusTxn.partialSignTxn({version, threshold, pks}, sk),
    };
}
...
/**
 * multisigAddress takes multisig metadata (preimage) and returns the corresponding human readable Timeus address.
 * @param version mutlisig version
 * @param threshold multisig threshold
 * @param addrs list of Timeus addresses
 */
